...
export class LittleMaskSeparately {
    ...
    nextSymbolSeparator(index: number): string | null {
        for (const [keys, value] of this.validators) {
            if (typeof keys === 'number' && index === keys) {
    ...
    private validSymbolDateMask(fullString: string, validators: Map<number[] | number, RegExp[][] | string>): boolean {
        for (let index = 0; index < fullString.length; index++) {
            const currentValidatorKey = Array.from(validators.keys()).find(
                (elem) =>
                    (typeof elem === 'number' && elem === index) || (typeof elem === 'object' && elem.includes(index))
            );
            if (!currentValidatorKey) return false;
            let subString = '';
            let currentIndexForMask = 0;
            if (typeof currentValidatorKey === 'number') {
                const currentValidator: string = validators.get(currentValidatorKey) as string;
                if (!this.isValidSeparator(currentValidator, fullString[index])) return false;
            } else {
                iteratorForSubstring: for (const j of currentValidatorKey) {
                    if (fullString[j]) {
                ...
                if (
                    !this.isValidGroup(
                        {
                            rules: (currentValidator as RegExp[][]) ?? [],
                            maxLengthMask: currentValidator?.[0].length || 0,
                        },
                        subString
                    )
                ) {
    ...
    private isValidSeparator(mask: string, value: string) {
        return mask === value;
    }
    ...
    private maskToRules(initialMask: RegExp | string): RegExp[][] {
        ...
        lineTraversal: for (let index = 0; index < mask.length; index++) {
            ...
            default:
                regular = !isString ? mask[index] : this.regularFromAlias(mask[index]);
            }
     ...
     */
    private regularFromBraces(index: number, mask: string, rules: RegExp[][], regular: string): [RegExp[][], number] {
        ...
        getSpec: for (let j = index + 1; j < mask.length; j++) {
            ...
            if (mask[j] === '}') {
                ...
                returnedRules = this.copyRulesWithNoRequiredSymbol(rules, regular, count, requiredCount);
    ...
    private copyRulesWithNoRequiredSymbol(
        ...
        rules.forEach((rule: RegExp[], ruleIndex: number) => {
            ...
            for (let slicedIndex = +count - requiredCount; slicedIndex > 0; slicedIndex--) {
    ...
    private setValidators(dateMask: ILittleMaskSeparately<TLtlPlainMask>) {
        ...
        for (let i = 0; i < dateMask.mask.length; i++) {
            ...
            if (!currentValidator) {
                ...
                // Арин, ты ебнутая?  Это  че такое?
                // this.validators.set([currentIndexForMask], [[new RegExp(`\\${dateMask.mask[i]}`)]]);
                this.validators.set(currentIndexForMask, dateMask.mask[i]);
                currentIndexForMask++;
            } else {
                const rules: RegExp[][] = this.rulesFromValidators(currentValidator);
